...
const EditAnime = () => {
  ...
  useEffect(() => {
    const fetchAnime = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/anime/${id}`, { withCredentials: true })
        setTitle(response.data.data.Title)
        setSynopsis(response.data.data.Synopsis)
        if (response.data.data.ReleaseDate) {
          setDate(response.data.data.ReleaseDate);
        } else {
          setDate(''); 
        }
        console.log("fetch anime success")
      } catch (error) {
  ...
  return (
    <div className="mt-5 flex-col max-w-screen pb-[20vh] min-h-screen flex items-center">
      <h1 className="text-center">Edit Anime</h1>
      <div className='w-[80vw] space-y-4'>
        ...
        <button onClick={() => navigate("/")} className="border-slate-300 border px-3 py-2 disabled:opacity-50 transition-all">
          cancel
        </button>
      </div>
    </div>
